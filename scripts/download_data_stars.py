...
if __name__ == "__main__":
    ...
    if not os.path.exists(f"data/{args.sim_name}_snap_{convert_float_to_three_digit(28)}.tar"):
        weblink = f"https://dataweb.cosma.dur.ac.uk:8443/eagle-snapshots//download?run={args.sim_name}&snapnum={28}"

        command = f"wget -P data/ --user={username} --password {password} --content-disposition '{weblink}'"

        os.system(command)

    print(f"Extracting data/{args.sim_name}_snap_{convert_float_to_three_digit(28)}.tar")
    os.system(f"tar -xvf data/{args.sim_name}_snap_{convert_float_to_three_digit(28)}.tar -C data") # -C hdf

    # extract_bh_data(nsnap = i)

    # print(f"Deleting data/{args.sim_name}_snap_{convert_float_to_three_digit(28)}.tar")
    # os.system(f"rm data/{args.sim_name}_snap_{convert_float_to_three_digit(28)}.tar")
